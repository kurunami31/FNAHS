...
-- ---------- 16 · membership fees ----------
-- The org sets one annual fee amount (app_settings.membership_fee_amount);
-- each payment is a fee_payments row marked FULL (100%) or HALF (50%) of it,
-- with an OR reference. A member is PAID once the sum of their payments
-- reaches the annual fee. Mirrors rbac.js 'fees.view' / 'fees.manage' scopes.
...
-- Events carry an optional fee amount; each attendee's payment is tracked
-- per event. Mirrors rbac.js 'events.manage' scope: officers who create and
-- manage events record/edit/void payments; members see their own rows.
...
-- ---------- 18 � event tallies ----------
-- Member-visible per-event attendance counts. Attendance rows are RLS-restricted
-- to the owner or door officers (is_door_officer), so a plain select can't show
-- members how many attended. This security-definer RPC returns just the totals so
-- the events page can display "N attended" without exposing who was there.
...
-- ---------- 19 AUDIT LOG ----------
-- Sensitive ops (role changes, fee records, event edits, member deletions) append a row here.
-- Rows are never written directly - the security-definer log_audit() RPC stamps the actor
-- from auth.uid(). Reading is limited to console officers via get_audit_logs().
